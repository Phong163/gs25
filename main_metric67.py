...
from config.params_metric67 import camera_configs
from rtsp_stream import RTSPStream
from torchreid.utils import FeatureExtractor
from datetime import datetime
logger = setup_logger(logger_name="gs25_metric67", log_file="output/gs25_metric67")
        ...
        for t_id, info in self.track_person.items():
            if self.current_frame - info["last_seen_frame"] > 200:
                if info["total_time"] > 10 and info["interacted_quantity"] > 0 and not info["reid"]:
                    send_time_to_kafka(6, self.box_id, self.camera_id, info["name_track_id"], info["interacted_quantity"])
