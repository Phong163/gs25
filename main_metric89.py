...
from config.params_metric89 import camera_configs
from rtsp_stream import RTSPStream
from torchreid.utils import FeatureExtractor
logger = setup_logger(logger_name="gs25_metric89", log_file="output/gs25_metric89")
...
class CustomerTracker:
    def __init__(self, store_id, camera_id, output_path, size=480, save_video=False, send_api=False):
        """Initialize the CustomerTracker with necessary components."""
        self.save_video = save_video
        self.send_api = send_api
        if store_id not in camera_configs:
            raise ValueError(f"No configuration found for store_id {store_id}")

        store_cfg = camera_configs[store_id]  # lấy config của store
        box_id = store_cfg["box_id"]

        if camera_id not in store_cfg["cameras"]:
            raise ValueError(f"No configuration found for camera_id {camera_id} in store_id {store_id}")

        config = store_cfg["cameras"][camera_id]

        self.box_id = box_id  # lưu box_id của store
    ...
    def process_frame(self, frame):
        ...
        for t_id, info in self.track_person.items():
            if self.current_frame - info["last_seen_frame"] > 200:
                if info["stopped_1"] and info["total_time_2"] > 0 and not info["reid"] and self.send_api:
                    if info["total_time_1"] < 2:
...
def run_tracker_for_camera(store_id,camera_id, output_path, size, save_video, send_api):
...
if __name__ == "__main__":
    ...
    camera_ids = args.camera_id if args.camera_id is not None else list(camera_configs[args.store_id]["cameras"].keys())

    threads = []
    for camera_id in camera_ids:
        thread = threading.Thread(
            target=run_tracker_for_camera,
            args=(args.store_id,camera_id, args.output, args.imgsz, args.save_video, args.send_api)
        )
